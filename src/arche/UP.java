 ...
 * License: GNU GPL
 *
 * $Id: UP.java,v 1.17 2003/05/08 12:24:54 sirna Exp $
 * $Revision: 1.17 $
...
public class UP extends PhysisVirtualMachine{
    ...
    //---THE VIRTUALMACHINE METHODS-------
    /**
     * When vivifying an organism its virtualmachine is just reset (not created).
     */
    public void reset(){
	/** Death condition: the genetic code doesn't contain the SEPARATOR. */
	...
	/** Death condition: There's no instruction after building the processor. */
	...
	try{
		...
		default:{
		    //The nop operation and the descriptors should end up here...
		    //System.out.println("Unexecuted instruction: " +  is.getInstructionMnemonic(insts[i]));
		    i++;
		}
		}//switch
		counter++;
		...
		//System.out.println("The tape: " + tape.toString());
	...
	int newpos = 0;
	//If we have enough operands in the instructionarray
	if (numops <= (insts.length - from)){
	    ...
	    for ( ; i < numops; i++){
		operands[i] = tape.read(IP.value); //TODO why not fetch?
		incrementIP(1);
	    }
	    newpos = insts.length;
	}
	//the operands should be positive numbers as they are indices - quite ugly
        // The modulo is needed! Consider the case of -32768 !!!
	...
	while (c != SEPARATOR){
		...
		i++;
		while(true){
		    //just skipping irrelevant symbols
		    while ( !isCodeForStructuralElement(c = tape.read(i)) ) {
			i++;
		    }
		    //if Stack then increase the size
		    if (c == S){
			size++;
			i++;
			c = tape.read(i);
		    }
		    //otherwise create the stack and quit S state.
		    else{
	...
	//DEATH CONDITION - if there's no instruction.
	if (!tape.contains(I)) {
	    bearer.kill();
	}
	int i = 0;
	...
	while ((c = tape.read(i)) != SEPARATOR){
	    if (c == I) {number_of_insts++;}
	    i++;
	}
	//and now we know how many instructions are needed, let's create the table
	instruction_table = new short[number_of_insts][];
	
	//scan the description again, and create the instructions
	i = 0;
	int state = BEGIN;
	int start = 0;
	int stop = 0;
	...
	//correcting the instruction values but not the operands
	int i = 0;
	while ( i < size){
    ...
    void setIP(int value) { IP.value = (short) value; }
	...
	if (!bearer.getMetabolism().isOutputFull()) {
	    ...
	    bearer.getMetabolism().clear(); //TODO what the hell is this?
	}
    }
	...
	setIP(ses[address % ssize].read()); //TODO modulo???
    }
	...
	if (!tape.isProliferationPossible()) {
	    //tape.clearExecutedFlag(IP.value);
	    incrementIP(1);
	    return;
	}
    ...
    //---SPECIAL LABELS FOR DESCRIPTION-----
    /** Register. */
    ...
    //----UTIL FUNCTIONS-----
    /* Returns true if the given instruction is structural instruction. */
	...
	sb.append("\b]");
	if (j != nextinst){ sb.append(" +" + (nextinst - j) + " operands");}
	return sb.toString();
    }
}
