 ...
 * License: GNU GPL
 *
 * $Id: EnvironmentImpl.java,v 1.5 2001/07/06 08:56:33 sirna Exp $
 * $Revision: 1.5 $
...
public class EnvironmentImpl implements Environment {
    ...
    public void setLifeSpace(LifeSpace lifespace_) { lifespace = lifespace_; }
    public LifeSpace getLifeSpace() { return lifespace; }
    ...
    public void proliferationPerformed(ProliferationEvent pe){
	    ...
	    //registering the geneinfo if it's enabled
	    if (Configuration.isGeneBankEnabled()){
		GeneBank.register(parent.getGenome(), pe.getNewSeed().getGenome());
	    }

	    //recalculating the parent's effective length (it might be changed by executing the child's code or inaccurate dividing.
	    parent.recalculateEffectiveLength();
	    parent.recalculateFitness();
	    //Log.status("" + parent.getFitness() +"___"+ parent.getVM().getGestationTime());
	    //Log.status("" + parent.getMerit() +"___");
     ...
     */
    public int getInputData() { return rnd.nextInt(input_data_higher_bound); }
